...
class DP {
     ...
     */
    static int bucoinChange (List<Coin> coins, int n) throws EmptyListE {//bottom up
    ...
    static boolean mpartition (List<Integer> s, int sum) {
        ...
        Pair<List<Integer>, Integer> pair = new Pair<>(s, sum);
        if (partitionMemo.containsKey(pair))return partitionMemo.get(pair);
        boolean value;
        try {
            if (sum == 0) {
                value = true;
            }
            else  value = ( mpartition(s.getRest(), sum - s.getFirst()) || mpartition(s.getRest(),sum));
        }catch (EmptyListE e){
            if (sum != 0){
                value = false;
            } else value = true;
    ...
    static int minDistance (List<BASE> dna1, List<BASE> dna2) {
        ...
        try{
            ...
            int p1 = pen + minDistance(dna1.getRest(), dna2.getRest());
            int p2_1 = GAP + minDistance(dna1.getRest(), dna2);
            int p2_2 = GAP + minDistance(dna1, dna2.getRest());

            int fin = Math.min(p1, Math.min(p2_1,p2_2));
            return fin;

        }catch (EmptyListE e){
            if (dna1.isEmpty()){
                return dna2.length()*GAP;
    ...
    static int mminDistance (List<BASE> dna1, List<BASE> dna2) {
        Pair<List<BASE>, List<BASE>> pair = new Pair<>(dna1, dna2);
        if (minDistanceMemo.containsKey(pair)) return minDistanceMemo.get(pair);
        ...
        try{
            ...
            int p1 = pen + mminDistance(dna1.getRest(), dna2.getRest());
            int p2_1 = GAP + mminDistance(dna1.getRest(), dna2);
            int p2_2 = GAP + mminDistance(dna1, dna2.getRest());

            fin = Math.min(p1, Math.min(p2_1,p2_2));
    ...
    static List<Character> lcs (List<Character> cs1, List<Character> cs2) {
        ...
        try{
            if (cs1.getFirst().equals(cs2.getFirst())){
                List<Character> c = lcs(cs1.getRest(), cs2.getRest());
                return new Node<>(cs1.getFirst(),c); //if first match then add to result

            }else{
                List<Character> r = lcs(cs1, cs2.getRest()); //omit first
                List<Character> l = lcs(cs1.getRest(), cs2);
    ...
    static List<Character> mlcs (List<Character> cs1, List<Character> cs2) {
        ...
        Pair<List<Character>, List<Character>> pList = new Pair<>(cs1, cs2);
        if (lcsMemo.containsKey(pList)) return lcsMemo.get(pList);
        List<Character> res;
        try{
            if (cs1.getFirst().equals(cs2.getFirst())){
                List<Character> c = mlcs(cs1.getRest(), cs2.getRest());
                res = new Node<>(cs1.getFirst(),c); //if first match then add to result

            }else{
                List<Character> r = mlcs(cs1, cs2.getRest()); //omit first
                List<Character> l = mlcs(cs1.getRest(), cs2);
                if (l.length() >= r.length()){
                    res = l;
                } else res = r;
            }
